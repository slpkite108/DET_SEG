...
def warm_up(
    ...
    for i, image_batch in enumerate(train_loader):
        ...
        seg = []
        for i in range(len(img_input[0])):
            seg.append(model(img_input[:,i])) 
...
def val_one_epoch(
    ...
    for i, image_batch in enumerate(val_loader):
        ...
        seg = []
        for i in range(len(image_batch["image"][0])):
            seg.append(inference(image_batch["image"][:,i],model)) 
    ...
    accelerator.print(f'inference_model current GPU memory usage: {torch.cuda.memory_allocated()/1024/1024} Mib')
    ...
    accelerator.print(f'inference_end current GPU memory usage: {torch.cuda.memory_allocated()/1024/1024} Mib')

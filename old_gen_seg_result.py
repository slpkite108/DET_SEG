# volume 개수 40개, exe export and run local
...
def gen_seg_result(
    ...
    for i, image_batch in enumerate(val_loader):
        ...
        save = os.path.join(f'{config.work_dir}','inference','gen',f'{config.finetune.checkpoint}',f'{("epoch_"+f"{config.inference.epoch:05d}") if isinstance(config.inference.epoch, int) else "best"}')

        if useNp:
            save = os.path.join(save,'np')
            
            os.makedirs(save,exist_ok = True)
            
            np.save(os.path.join(save,'inf_image_'+str(i).zfill(2)+'.npy'), img)
            np.save(os.path.join(save,'inf_lab1_'+str(i).zfill(2)+'.npy'), lab1_gen)
            np.save(os.path.join(save,'inf_lab2_'+str(i).zfill(2)+'.npy'), lab2_gen)
            np.save(os.path.join(save,'inf_lab1_gt_'+str(i).zfill(2)+'.npy'), lab1_gt)
            np.save(os.path.join(save,'inf_lab2_gt_'+str(i).zfill(2)+'.npy'), lab2_gt)
        else:
            save = os.path.join(save,'nrrd')
            
            os.makedirs(save,exist_ok = True)
            
            sitk.WriteImage(sitk.GetImageFromArray(img), os.path.join(save,'inf_image_'+str(i).zfill(2)+'.nrrd'))
            sitk.WriteImage(sitk.GetImageFromArray(lab1_gen.astype(int)),os.path.join(save,'inf_lab1_'+str(i).zfill(2)+'.nrrd'))
            sitk.WriteImage(sitk.GetImageFromArray(lab2_gen.astype(int)), os.path.join(save,'inf_lab2_'+str(i).zfill(2)+'.nrrd'))
            sitk.WriteImage(sitk.GetImageFromArray(lab1_gt.astype(int)), os.path.join(save,'inf_lab1_gt_'+str(i).zfill(2)+'.nrrd'))
            sitk.WriteImage(sitk.GetImageFromArray(lab2_gt.astype(int)), os.path.join(save,'inf_lab2_gt_'+str(i).zfill(2)+'.nrrd'))
...
if __name__ == "__main__":
    ...
    logging_dir = os.path.join(f'{config.work_dir}','inference','gen',f'{config.finetune.checkpoint}',f'{("epoch_"+f"{config.inference.epoch:05d}") if isinstance(config.inference.epoch, int) else "best"}',f'{str(datetime.now())}')
    ...
    elapsed = done - start
    print(time.strftime('runtime: %Ss', time.localtime(elapsed)))

    sys.exit(0)

"""
def save_label(savePath, imageList:list, mode:str = None, separate:bool = False):
    #imageList는 image dict를 요소로 갖는다. image = {str,np.array} 형태로 저장
    if separate:
        labelNum = 2
    else:
        labelNum = 1
    
    if mode == None or mode == 'numpy':
